...
class GaussianVAE(MLP):
    def __init__(self, input_data, input_size,
        ...
        with tf.compat.v1.name_scope('encoder-parameter'):
            self.encoder_parameter = self.encoder()

        with tf.compat.v1.name_scope('sample'):
            self.ep = tf.random.normal(
    ...
    def encoder(self, prob=0.1): # changed to 0.1 in TF2.x
        weights_mu = tf.nn.dropout(self.weights_mu, 1 - (1 - (prob)))
