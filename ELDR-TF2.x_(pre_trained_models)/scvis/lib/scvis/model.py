...
class SCVIS(object):
    def __init__(self, architecture, hyperparameter):
        ...
        tf.compat.v1.set_random_seed(self.hyperparameter['seed'])

        # Place_holders
        self.batch_size = tf.compat.v1.placeholder(dtype=tf.int32)
        self.x = tf.compat.v1.placeholder(tf.float32, shape=[None, self.architecture['input_dimension']])
        self.z = tf.compat.v1.placeholder(tf.float32, shape=[None, self.architecture['latent_dimension']])

        self.p = tf.compat.v1.placeholder(tf.float32, shape=[None, None])
        self.iter = tf.compat.v1.placeholder(dtype=tf.float32)
        ...
        with tf.compat.v1.name_scope('ELBO'):
            self.weight = tf.clip_by_value(tf.reduce_sum(input_tensor=self.p, axis=0), 0.01, 2.0)

            self.log_likelihood = tf.reduce_mean(input_tensor=tf.multiply(
            ...
            self.kl_divergence = \
                tf.reduce_mean(input_tensor=0.5 * tf.reduce_sum(input_tensor=self.latent['mu'] ** 2 +
                                                   self.latent['sigma_square'] -
                                                   tf.math.log(self.latent['sigma_square']) - 1,
                                                   axis=1))
        ...
        with tf.compat.v1.name_scope('optimizer'):
            ...
            if self.hyperparameter['optimization']['method'].lower() == 'adagrad':
                self.optimizer = tf.compat.v1.train.AdagradOptimizer(learning_rate)
    ...
    def tsne_repel(self):
        ...
        sum_y = tf.reduce_sum(input_tensor=tf.square(self.z_batch), axis=1)
        ...
        p = self.p + 0.1 / self.n
        p = p / tf.expand_dims(tf.reduce_sum(input_tensor=p, axis=1), 1)

        num = tf.pow(1.0 + num, -(nu + 1.0) / 2.0)
        attraction = tf.multiply(p, tf.math.log(num))
        attraction = -tf.reduce_sum(input_tensor=attraction)

        den = tf.reduce_sum(input_tensor=num, axis=1) - 1
        repellant = tf.reduce_sum(input_tensor=tf.math.log(den))
    ...
    def encode(self, x):
        var = self.vae.encoder(prob=0.0) # changed in TF2.0 to 0.0
        feed_dict = {self.x: x}
    ...
    def get_elbo(self, x):
        ...
        kl_divergence = tf.reduce_mean(input_tensor=0.5 * tf.reduce_sum(input_tensor=self.latent['mu'] ** 2 +
                                                           self.latent['sigma_square'] -
                                                           tf.math.log(self.latent['sigma_square']) - 1,
                                       axis=1))
