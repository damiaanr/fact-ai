...
class Explainer:
    ...
    minimum_iters = 10000 # except if not a minimum of Y iters has been done
    consecutive_steps = 10 # in negative gradient direction per sampling
    loss_discount = 0.99
    tol = 0.0001

    def __init__(self, X, Y, transformer, num_clusters):
    ...
    def plot_metrics(self, fontsize = 55, labelsize = 40):
        ...
        plt.savefig("plots/metrics_plot_" + str(datetime.timestamp(datetime.now())) + ".png")
        plt.show()
        plt.close()
    ...
    def plot_explanation(self, c1, c2, k = None, num_points = 50, feature_names = None):
        ...
        plt.savefig("plots/explanation_plot_" + str(datetime.timestamp(datetime.now())) + ".png")
        plt.show()
        plt.close()

with torch.no_grad():
    # Input set (currently 'Heart')
    x = pd.read_csv("Data/X.tsv", sep="\t").values # Heart input
    ...
    # Standardizing - not sure if necessary/good
    x_batch = torch.from_numpy(x).float()
    encoder_mu, encoder_log_var = net.encoder(x_batch)
    X_net = net.sampling(encoder_mu, encoder_log_var)
    X_net.detach().numpy()

    # K-means clustering in latent space
    kmeans = KMeans(n_clusters=8, random_state=42)
    kmeans.fit(X_net)
    y_kmeans = kmeans.predict(X_net)

    # Plotting for interest

    plt.scatter(X_net[:, 0], X_net[:, 1], c=y_kmeans, s=2, cmap='viridis')
    centers = kmeans.cluster_centers_
    plt.scatter(centers[:, 0], centers[:, 1], c='black', s=200, alpha=0.5);
    plt.show()


    # Optionally we could plot plumb's clusters, but they correspond to a different latent space
    # with open('Data3/plumb-classes.txt', 'r') as lc:
    #  y_plumb = [[int(num[0:-1]) for num in line.split()] for line in lc]
    # plt.scatter(X_pca[:, 0], X_pca[:, 1], c=y_plumb, s=20, cmap='viridis')

    # Transform function
    transform_function = (lambda x: transform(x, net))

    def transform(x, net):
        x_batch = torch.from_numpy(x)
        encoder_mu, encoder_log_var = net.encoder(x_batch)
        return net.sampling(encoder_mu, encoder_log_var)

    E = Explainer(x, y_kmeans, transform_function, 8)

    E.learn()
    cr, cv = E.metrics()
    E.plot_metrics()
    E.plot_explanation(2, 6, feature_names=["age", "sex", "cp", "trestbps", "chol", "fbs", "restecg", "thalach", "exang", "oldpeak", "slope", "ca", "thal"])
    E.plot_explanation(2, 5, feature_names=["age", "sex", "cp", "trestbps", "chol", "fbs", "restecg", "thalach", "exang", "oldpeak", "slope", "ca", "thal"])
    E.plot_explanation(3, 4, feature_names=["age", "sex", "cp", "trestbps", "chol", "fbs", "restecg", "thalach", "exang", "oldpeak", "slope", "ca", "thal"])
    E.plot_explanation(1, 5, feature_names=["age", "sex", "cp", "trestbps", "chol", "fbs", "restecg", "thalach", "exang", "oldpeak", "slope", "ca", "thal"])
    E.plot_explanation(1, 6, feature_names=["age", "sex", "cp", "trestbps", "chol", "fbs", "restecg", "thalach", "exang", "oldpeak", "slope", "ca", "thal"])
    E.plot_explanation(5, 3, feature_names=["age", "sex", "cp", "trestbps", "chol", "fbs", "restecg", "thalach", "exang", "oldpeak", "slope", "ca", "thal"])
