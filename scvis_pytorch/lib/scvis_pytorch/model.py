...
class SCVIS(nn.Module):
    def __init__(self, architecture, hyperparameter):
        ...
        # Deleted: tf.reset_default_graph()
        # Deleted: self.sess = tf.InteractiveSession()
        self.normalizer = torch.tensor(1.0, requires_grad=False) # Replaces: self.normalizer = tf.Variable(1.0, name='normalizer', trainable=False)
