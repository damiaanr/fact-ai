...
class MLP(nn.Module):
    def __init__(self, input_data, input_size, layer_size, output_dim,
        ...
        self.activate, self.init_w, self.init_b = activate_op, init_w_op, init_b_op
...
class GaussianVAE(MLP):
    def __init__(self, input_data, input_size,
        ...
        super(self.__class__, self).__init__(input_data, input_size, layer_size, output_dim)
        ...
        with tf.name_scope('sample'):
            self.ep = torch.distributions.normal(
                size=[self.input_size, self.output_dim],
                mean=0, stddev=1, name='epsilon_univariate_norm')

            self.z = torch.add(self.encoder_parameter.mu,
                               torch.sqrt(self.encoder_parameter.sigma_square) * self.ep, name='latent_z')
        ...
        with tf.name_scope('decoder'):
            ...
            self.decoder_hidden_layer_out = self.activate(
                torch.matmul(self.z, self.weights[-1]) + self.biases[-1]
            )
            ...
            for in_dim, out_dim in zip(self.decoder_layer_size, self.decoder_layer_size[1:]):
                ...
                self.decoder_hidden_layer_out = self.activate(
                    torch.matmul(self.decoder_hidden_layer_out, self.weights[-1]) + self.biases[-1]
                )
            ...
            mu = torch.add(torch.matmul(self.decoder_hidden_layer_out, self.decoder_weights_mu),
                           self.decoder_bias_mu)
            sigma_square = torch.add(torch.matmul(self.decoder_hidden_layer_out, self.decoder_weights_sigma_square),
                                     self.decoder_bias_sigma_square)

            clamped_values = torch.clamp(tf.nn.softplus(sigma_square), EPS, MAX_SIGMA_SQUARE)
            self.decoder_parameter = LocationScale(mu, clamped_values)
