...
def train_class(x, y,
    ...
    model_lm = LogisticRegression().fit(x_train, y_train)
    print("\nLM Accuracy: ", accuracy_score(model_lm.predict(x_test), y_test), "\n")

    model_rf = RandomForestClassifier(n_estimators = 10).fit(x_train, y_train)
    print("\nRF Accuracy: ", accuracy_score(model_rf.predict(x_test), y_test), "\n")
    ...
    Y = tf.compat.v1.placeholder("float", [None, 1], name = "Y_in")
    ...
    model_loss = tf.reduce_mean(input_tensor=tf.nn.sigmoid_cross_entropy_with_logits(labels = Y, logits = pred))
    tf.compat.v1.summary.scalar("Cross Entropy", model_loss)
    
    accuracy = tf.reduce_mean(input_tensor=tf.cast(tf.equal(tf.round(tf.nn.sigmoid(pred)), Y), tf.float32))
