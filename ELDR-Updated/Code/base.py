...
class MLP():
    ...
    def __init__(self, shape):
        self.shape = shape
        self.weight_init = tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution="uniform")
        self.bias_init = tf.compat.v1.constant_initializer(0.0)
    
    def layer(self, input, input_size, output_size):
        weights = tf.compat.v1.get_variable("weights", [input_size, output_size], initializer = self.weight_init)
        biases = tf.compat.v1.get_variable("biases", output_size, initializer = self.bias_init)
    ...
    def model(self, input):
        ...
        x = input
        for i in range(n - 2):
            with tf.compat.v1.variable_scope("hidden_" + str(i + 1)):
                out = self.layer(x, shape[i], shape[i + 1])
                x = out
        with tf.compat.v1.variable_scope("output"):
            weights = tf.compat.v1.get_variable("weights", [shape[n - 2], shape[n - 1]], initializer = self.weight_init)
            biases = tf.compat.v1.get_variable("biases", shape[n - 1], initializer = self.bias_init)
